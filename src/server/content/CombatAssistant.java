	...
	*/
	public void attackNpc(int i) {		
		if (Server.npcHandler.npcs[i] != null) {
			...
			if (Server.npcHandler.npcs[i].npcType == 199) {
				return;
			}
	...
	public void delayedHit(int i) { // npc hit delay
		if (Server.npcHandler.npcs[i] != null) {
			...
			c.getPA().sendSound(CombatSounds.getNpcBlockSound(NPCHandler.npcs[c.oldNpcIndex].npcType), 100, 0);
		...
		if (Server.playerHandler.players[playerId] != null) {
			...
			if (checkMultiBarrageReqs(playerId)) {
				...
				if (Misc.random(mageAtk()) > Misc.random(mageDef()) && !c.magicFailed) {
					...
					c2.gfx100(85);
					c.getPlayerAssistant().sendSound(SoundList.MAGE_FAIL, 100, 0);
				}			
			}		
		}	
	}
