...
public class PartyRoom {
	...
	public static void dropAll() {
		...
		for (int x = 0; x < roomItems.length; x++) {
			if (roomItemsN[x] > 0) {
				...
				do {
					...
					trys++;
				} while (getCoords().contains(Balloons.getCoords()) && trys < 100);
		...
		for (int x = 0; x < amount * 2; x++) {
			...
			do {
				o = Balloons.getEmpty();
			} while (getCoords().contains(new Point(o.objectX, o.objectY))
					&& trys < 100);
			if (trys > 100) {
				break;
			}
	...
	public static void setCoords(ArrayList<Point> coords) {
		PartyRoom.coords = coords;
	}
}
