...
public class RangersGuild {
	...
	public void fireAtTarget() {
		...
		if (arrowsLeft != 0) {
			final int[] BOWS = { 9185, 839, 845, 847, 851, 855, 859, 841,
	...
	public void exchangePoints() {
		if (arrowsLeft == 0 && playerScore > 0) {
			if(c.getItemAssistant().freeSlots() > 0) {
				...
				c.getDH().sendNpcChat2("Well done. Your score is : " + playerScore + ".", "You have earned " + ticketsAmt + " Archery tickets.", c.talkingNpc, "Tutor");
				playerScore = 0;
				c.nextChat = 0;
			} else {
			...
			c.getDH().sendStatement("You still have " + arrowsLeft + " shots left.");
			c.nextChat = 0;
		}
	}
	...
	public void howAmIDoing() {
		if (playerScore == 0) {
			c.getDH().sendNpcChat2("You haven't started yet. Stand behind the hay bales and", "shoot those arrows at the targets.", c.talkingNpc, "Tutor");
			c.nextChat = 0;
		} else {
			c.getDH().sendNpcChat2("Your score is : " + playerScore, "Your doing very well!", c.talkingNpc, "Tutor");
			c.nextChat = 0;
		}
	}
