...
public class ShopAssistant {
	...
	**/
	...
	public void buyFromShopPrice(int removeId, int removeSlot){
		...
		if (c.myShopId == 112) {
			c.sendMessage(c.getItems().getItemName(removeId)+": currently costs "+getCastleItemValue(removeId)+" tickets");
		} else if (c.myShopId == 150) {
			c.sendMessage(c.getItems().getItemName(removeId)+": currently costs "+getPKItemValue(removeId)+" PK points");
		} else if (c.myShopId == 151) {
			c.sendMessage(c.getItems().getItemName(removeId)+": currently costs "+getSlayerItemValue(removeId)+" Slayer points");
		} else {
	...
	public int getSlayerItemValue(int id) {
		return 0;
	}
	
	public int getPKItemValue(int id) {
		switch (id) {
			case 6889:
			case 6914:
			return 200;
	...
	**/
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0) {
			...
			int Slot = 0;
			int Slot1 = 0;//Tokkul
			int Slot2= 0;//cw ticket
			int Slot3= 0;//pk points
			int Slot4= 0;//Slayer points
		
			for (int i = amount; i > 0; i--) {
				if (c.myShopId == 112) {
				TotPrice2 = (int)Math.floor(getCastleItemValue(itemID));
				} else {
				TotPrice2 = (int)Math.floor(getItemShopValue(itemID, 0, fromSlot));	
				}
				Slot = c.getItems().getItemSlot(995);
				Slot1 = c.getItems().getItemSlot(6529);
				Slot2 = c.getItems().getItemSlot(4067);//cw ticket
				Slot3 = c.pkPoints;
				Slot4 =c.slayerPoints;
                if(TotPrice2 <= 1) {
                	TotPrice2 = (int)Math.floor(getItemShopValue(itemID, 0, fromSlot));
                	TotPrice2 *= 1.66;
                }
				...
				if (Slot3 == -1) {
					...
					if (c.playerItemsN[Slot2] >= TotPrice2) {
						if (c.getItems().freeSlots() > 0) {
							c.getItems().deleteItem(Slot2, c.getItems().getItemSlot(Slot2), TotPrice2);
