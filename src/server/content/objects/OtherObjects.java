...
public class OtherObjects {
	...
	private static void object(Client player, int id, int x, int y) {
		player.getPA().object(id, x, y, 0, ObjectHandler.getObjectFace(player, id), 10);
	}
	...
	private static void handleSpecialObject(final Client player, final int objectType) {	
		String objectName = ObjectDef.getObjectDef(objectType).name;
			if (objectType == 160 && player.absX == 3096) {
					...
					player.sendMessage("You search the " + objectName.toLowerCase() + "...");
					CycleEventHandler.addEvent(player, new CycleEvent() {
						@Override
						public void execute(CycleEventContainer container) {
							if (random == 1) {
								player.getDH().sendStatement2("Wow! A needle!", "Now what are the chances of finding that?");
								player.nextChat = 0;
