...
import server.players.Position;
...
public class PassDoor {
	...
	public static boolean passThroughDoor(final Client client, final int objectType, int face1, final int face2, final int type, int x, int y, final int height) {
		if (System.currentTimeMillis() - doorDelay < 1200) {
			client.sendMessage("You must wait longer to pass this door.");
			return false;
		}
		client.getPA().object(objectType, client.objectX, client.objectY, height, face1, type);
		client.getPlayerAssistant().walkTo(x, y);
		client.stopPlayer = true;
		CycleEventHandler.addEvent(client, new CycleEvent() {
			@Override
			public void execute(CycleEventContainer container) {
				client.getPA().object(objectType, client.objectX, client.objectY, height, face2, type);
				container.stop();
			}
