...
public class FreakyForester {
	...
	public static void teleportToLocation(Client client) {
		...
		client.sendMessage("Talk to the freaky forester to get out.");
	}
	...
	public static void leaveArea(Client client) {
		if (client.killedPheasant[client.getPheasent]) {
			...
			client.sendMessage("Congratulations, you've completed the freaky forester event!");
			if (client.recievedReward == false) {
				...
				client.sendMessage("You have already beat the freaky forester event so you get 500 coins.");
			}
