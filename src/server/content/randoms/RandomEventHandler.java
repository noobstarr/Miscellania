...
public class RandomEventHandler {
	...
	public static void failEvent(Client player) {
		...
		player.sendMessage("You wake up in a strange location...");
		resetEvent(player);
		player.getPlayerAssistant().closeAllWindows();
	}

	public static void callRandom(Client player) { // add all random events here
		if (player.inFightCaves() || player.playerEquipment[player.playerWeapon] == 4024 || player.newPlayer) {
			return;
		}
	...
	public static void addRandom(Client player) {
		if (player.randomActions >= CALL_RANDOM) {
			callRandom(player);
			if (player.isShopping || player.inTrade || player.inDuel || player.isBanking || player.duelStatus == 1 && !player.hasSandwhichLady) {
				player.getPlayerAssistant().closeAllWindows();
			}
		} else {
			int nextRandom = CALL_RANDOM - player.randomActions;
			if (player.playerRights == 3) {
				player.sendMessage("Next random will be in " + nextRandom + " more random actions.");
			}
