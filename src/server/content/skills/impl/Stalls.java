...
public class Stalls {
	...
	public static void attemptStall(final Client c, final int objectId, final int x, final int y) {
		c.sendMessage(Integer.toString(objectId));
		for (final stallData s : stallData.values()) {
			if (System.currentTimeMillis() - c.lastThieve < 2500 + r(2500)) {
				//c.sendMessage("You need to wait longer before you can thieve this stall!");
				return;
			}
			...
			if(objectId == s.getObject()) {
				...
				if (c.playerLevel[c.playerThieving] >= s.getLevel()) {
					...
					if (x == 2656 && objectId == 2560) {
					Server.objectHandler.createAnObject(c, 634, x, y, 1);
					} else {
					...
					c.sendMessage("You steal a "+ItemAssistant.getItemName(random[0])+" from the stall.");
					CycleEventHandler.addEvent(c, new CycleEvent() {
						@Override
						public void execute(CycleEventContainer container) {
							if (x == 2656 && objectId == 2560) {
							Server.objectHandler.createAnObject(c, s.getObject(), x, y, 1);
							} else {
