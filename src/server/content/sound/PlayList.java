...
public class PlayList {
	...
	public void fixAllColors() {
		for (int i = 0; i < 385; i++) {
			if (Songs.get(i) != null) {
				player.getPlayerAssistant().sendColor(Songs.get(i).childId,
						255 << 10 | 0 << 5 | 0);
	...
	public void updateList(int id) {
		player.getPlayerAssistant().sendColor(id, 0 << 10 | 255 << 5 | 0);
	}
	...
	public void handleButton(int button) {
		...
		if (button == 38197) {
			loop = !loop;
		} else if (button == 24125) {
			auto = true;
			player.getPlayerAssistant().sendFrame36(18, 1);
		} else if (button == 24126) {
			auto = false;
			player.getPlayerAssistant().sendFrame36(18, 0);
		} else {
			...
			if (songId == -1) {
				...
				player.getPlayerAssistant().sendSong(songId);
				player.getPlayerAssistant().sendFrame126(
						names[Songs.get(songId).arraySlot], 4439);
				updateList(4439);
			}
